...
def toy_transactions_path(tmp_path):
    ...
    forward-revenue window summation (both boundary directions), the
    corrupted expire_dt < txn_dt guard, and ref_date_max_cutoff exclusion.
    See test_labels.py for the hand-computed expected value of every row.
    """
    cols = [
    ...
    rows = [
        # user_churn_clean: one paid cycle, no renewal -> churned, zero forward revenue
        ["user_churn_clean", 1, 30, 100, 100, 1, 20161101, 20161201, 0],

        # user_renew_clean: renews 4 days after expiry (well within 30-day
        # grace) and within the 59-day forward-revenue window -> not churned,
        # fwd_rev_59d=150. The renewal's own expiry (2017-01-05) is past the
        # cutoff so it isn't itself a ref_date candidate - ref_date stays at
        # 2016-12-01.
        ...
        # user_fwd_rev_boundary: churns per the 30-day rule (win-back
        # transaction arrives well after the grace window), but the win-back
        # purchase at exactly ref_date+59 must still count toward forward
        # revenue (inclusive upper bound), while a second purchase at
        # ref_date+60 must not.
        ["user_fwd_rev_boundary", 1, 30, 100, 100, 1, 20161101, 20161201, 0],
        ["user_fwd_rev_boundary", 1, 30, 200, 200, 1, 20170129, 20170228, 0],
        ["user_fwd_rev_boundary", 1, 30, 300, 300, 1, 20170130, 20170301, 0],
