...
Predicts subscriber churn and near-term revenue for KKBox (WSDM 2017 Kaggle challenge) subscribers. The primary model is a **CatBoost churn classifier + forward-revenue regressor, plus a CatBoost Cox proportional-hazards survival model** (`03a_CatBoost_and_Cox_Models.ipynb`). A **Zero-Inflated LogNormal (ZILN) neural net** was built and directly compared against it as a dedicated forward-revenue challenger (`03c_ZILN_ForwardRevenue.ipynb`, 5-seed ensemble, Optuna-tuned, same feature set) rather than assuming CatBoost superior — **CatBoost won on every metric**; see the comparison below. Built from [`MultiTask_MLP_ChurnLTV_ProjectPlan_v2.pdf`](MultiTask_MLP_ChurnLTV_ProjectPlan_v2.pdf).
